...
def validate_production_config():
    ...
    if "postgres:postgres" in settings.DATABASE_URL:
        errors.append(
            "DATABASE_URL contains default credentials - must be changed for production"
        )

    if settings.S3_ACCESS_KEY == "minioadmin":
        errors.append(
            "S3_ACCESS_KEY is using default value - must be changed for production"
        )

    if settings.S3_SECRET_KEY == "minioadmin":
        errors.append(
            "S3_SECRET_KEY is using default value - must be changed for production"
        )
