...
class Location(Base):
    ...
    path: Mapped[str] = mapped_column(
        String, index=True, default="/"
    )  # Materialized path e.g. /1/5/
...
class Item(Base):
    ...
    pending_changes: Mapped[dict] = mapped_column(
        JSON, server_default="{}"
    )  # Stores AI suggestions for review
    ...
    if "sqlite" not in settings.DATABASE_URL and not os.environ.get("TEST_MODE"):
        search_vector: Mapped[str] = mapped_column(
            TSVECTOR,
            Computed(
                "to_tsvector('english', coalesce(name, '') || ' ' || coalesce(data::text, ''))",
                persisted=True,
            ),
        )
        __table_args__ = (
...
class Media(Base):
    ...
    metadata_json: Mapped[dict] = mapped_column(
        JSON, server_default="{}"
    )  # Renamed to avoid confusion with MetaData
    created_at: Mapped[datetime] = mapped_column(server_default=func.now())
