...
async def start_companion_session():
    ...
    data = {"status": "waiting", "created_at": str(uuid.uuid1())}
...
async def companion_page(request: Request, session_id: str):
    ...
    if not redis_conn.exists(key):
        return HTMLResponse(
            "<h1>Session Expired</h1><p>Please start a new session on your desktop.</p>",
            status_code=404,
        )
...
async def companion_upload(
    session_id: str, photo: UploadFile = File(...), db: Session = Depends(get_db)
):
    ...
    # Create Draft Item
    item = Item(name=None, is_draft=True, category_id=None)  # Draft
    db.add(item)
    db.flush()  # Get ID
    ...
    loc_id = (
        default_loc.id if default_loc else 1
    )  # Fallback, might error if foreign key fails
    ...
    media = Media(item_id=item.id, type="image", s3_key=s3_key)
    db.add(media)
    ...
    try:
        ...
        logger.exception(
            f"Failed to enqueue process_item_image job for item {item.id} in companion mode"
        )
